...
# Put par bond coupons into the parms_df
    ...
    util.export_to_csv(dataframes, names, folder_path, estfile)

    return parbd_rateprice_df, zerobd_rateprice_df, annuity_rateprice_df, pbparms_df


def seperate_pb_zb_anty_wrapper(parbd_rateprice_df, zerobd_rateprice_df, annuity_rateprice_df):  # , estfile, output_dir
    ...
    parbd_rate['Max'] = parbd_rate.max(axis=1)
    parbd_rate['Min'] = parbd_rate.min(axis=1)
    zerobd_rate['Max'] = zerobd_rate.max(axis=1)
    zerobd_rate['Min'] = zerobd_rate.min(axis=1)
    annuity_rate['Max'] = annuity_rate.max(axis=1)
    annuity_rate['Min'] = annuity_rate.min(axis=1)
...
def find_max_min_pb_rate(pzb_rate):
    ...
    pzb_rate= pzb_rate.drop(['year', '5_year_bin'], axis=1)
...
def calc_par_total_ret(curve_df, table_breaks_yr, twostep=False, parmflag=True, padj=False):
    ...
    table_breaks_yr = [value for value in table_breaks_yr if (value >= 1/3) or (value < 0.1)]
    ...
    for index, row in curve_df_lag.iterrows():
        ...
        parms_df_pre_ind = parms_df_pre_ind[(parms_df_pre_ind.index.get_level_values('type') == 'parbond')]
        dirtyprice = pv.pvBondFromCurve(curve, settledate = curve[1], parms = parms_df_pre_ind)
#        pbrates, cleanprice, dirtyprice = parbond_rates(curve, parms_df_pre_ind, twostep=twostep, parmflag=parmflag, padj=padj)
        total_ret = (dirtyprice[:,1]/100) - 1
...
#    AD_values = np.array([calculate_ad(qd) for qd in quotedate_ind])
    ...
    yld_ret_df = parbd_rate_df.copy()
    yld_ret_df = yld_ret_df[yld_ret_df.index.get_level_values('quotedate') != int(quotedates[-1])]
    ctype_ind = yld_ret_df.index.get_level_values('ctype').unique()
    yld_ret_df['AD'] = np.tile(AD_values,len(ctype_ind))

    for column in breaks_cols:
        yld_ret_df[column] = yld_ret_df.apply(lambda row: yld_return(row[column], row['AD']), axis=1)
    yld_ret_df.drop(columns='AD', inplace=True)
    yld_ret_df.rename_axis(index={'rtype': 'return_type'}, inplace=True)
    yld_ret_df = yld_ret_df.reset_index()
    yld_ret_df['return_type'] = 'yield_return'
    yld_ret_df['quotedate'] = xtrindex('quotedate')
    yld_ret_df.columns = columns      # To make sure names consistent
    yld_ret_df.set_index(['ctype','quotedate','return_type'], inplace=True, drop=True)

    total_ret_prc_df.iloc[:, 0] = yld_ret_df.iloc[:, 0]

    x1 = np.asarray(total_ret_prc_df.values.tolist())   # array of total returns
    x2 = np.asarray(yld_ret_df.values.tolist())   # array of yield returns
    x3 = (1 + x1) / (1 + x2) - 1.0            # return in excess of yield
    columns1 = [f'{yr}YR' for yr in table_breaks_yr]
    ex_return_df = pd.DataFrame(x3.tolist(),columns=columns1)
    ex_return_df.iloc[:, 0] =  0
    ex_return_df.insert(0,'return_type','yield_excess')
    ex_return_df.insert(0,'quotedate',xtrindex('quotedate'))
    ex_return_df.insert(0,'ctype',xtrindex('ctype'))
    ex_return_df.set_index(['ctype','quotedate','return_type'], inplace=True, drop=True)
    return_df = pd.concat([total_ret_prc_df,yld_ret_df,ex_return_df])
...
def plot_singlecurve(output_dir, estfile, curve, plot_points_yr, rate_type = 'parbond', opt_type='LnY', 
                     yield_to_worst=True, yvols=0, df_price_yield=None, pbtwostep=False, parmflag=True, 
                     padj=False, sqrtscale=False):
    """
...
# Create df of bond parameters
    ...
    if not yield_to_worst:
        plt.title(f'{rate_type.capitalize()} Curve, {date_format}, {opt_type}, vol={yvols}')
    else:
        plt.title(f'{rate_type.capitalize()} Curve, {date_format}, {opt_type}, Yield-to-Worst')

    plt.ylabel('Rates')
    # Create directory if it does not exist
    full_path = f'{output_dir}/{estfile}/{rate_type}'
    os.makedirs(full_path, exist_ok=True)

    plt.savefig(f'{full_path}/{int(quote_date_ymd)}_{rate_type}.png')
    plt.show()

 

def plot_singlecurve_act_pred(output_dir, estfile, curve, plot_points_yr, rate_type='parbond', opt_type='LnY', 
                              yield_to_worst=True, yvols=0, df_price_yield=None, pbtwostep=False, parmflag=True, 
                              padj=False, sqrtscale=False, durscale=False):
    """
...
#        xplot_points_yr = acprice_df.iloc[0, 2:]   #  This is not correct formula, but as of 29-jul-24 annuity rates are wrong
    ...
    if yield_to_worst==False:
        plt.title(f'{rate_type.capitalize()} Curve, {date_format}, {opt_type}, vol={yvols} (in %)')
    else:
        plt.title(f'{rate_type.capitalize()} Curve, {date_format}, {opt_type}, Yield-to-Worst (in %)')
    plt.ylabel('Rates')


    # Create directory if it does not exist
    if durscale == True:
        full_path = f'{output_dir}/{estfile}/{rate_type}_w_act_dur'
    else:
        full_path = f'{output_dir}/{estfile}/{rate_type}_w_act'
    os.makedirs(full_path, exist_ok=True)

    plt.savefig(f'{full_path}/{int(quote_date_ymd)}_{rate_type}_w_act.png')
    plt.show()
...
# This function should be very similar to plot_singlecurve_act_pred--could delete the previous one
def plot_pbcurve(output_dir, estfile, curve, plot_points_yr, rate_type='parbond', opt_type='LnY', yield_to_worst=True,
...
# Create df of bond parameters
    parms_df = create_parbnd_df(curve_df, plot_points_yr,addmths=True)  # add by months
    rateprice_df, parms_df = produce_pb_zb_anty_dfs(curve_df, parms_df, rate_type, pbtwostep, parmflag, padj)
    rate_df = rateprice_df.xs('rate', level=2)
    if durscale:   # need to get duration of par bonds, but this is the PV of an annuity which we can easily calculate
#        arate_df, acprice_df, adprice_df = produce_pb_zb_anty_dfs(curve_df, plot_points_yr, 'annuity', pbtwostep, parmflag, padj)
...
#        xplot_points_yr = acprice_df.iloc[0, 2:]   #  This is not correct formula, but as of 29-jul-24 annuity rates are wrong
    ...
    if not fortran:
        ...
        plt.title(f'{rate_type.capitalize()} Curve, {date_format} (in %)')
...
# def create_animation_from_images(image_folder, output_dir, estfile, crvtype, file_type='pbrate', 
#                                  fps=2, output_format='gif'):
    
#     # Get the list of images sorted by filename
#     images = sorted([img for img in os.listdir(image_folder) if img.endswith('.png')])

#     first_image = plt.imread(os.path.join(image_folder, images[0]))
#     fig, ax = plt.subplots()
...
#         output_dir = f'{output_dir}/{file_type}_animation_{estfile}_{crvtype}.gif'
#         anim.save(output_dir, writer=PillowWriter(fps=fps))
#     # elif output_format == 'mp4':  # could not work properly
#     #     output_dir = f'{output_dir}/{file_type}_animation_{estfile}_{crvtype}.mp4'
#     #     anim.save(output_dir, writer=FFMpegWriter(fps=fps))

#     plt.close()



def create_animation_from_images(image_folder, output_dir, estfile, file_type='pbrate', 
                                 fps=2, output_format='gif'):
    
    images = sorted([img for img in os.listdir(image_folder) if img.endswith('.png')])
    image_list = [imageio.imread(os.path.join(image_folder, img)) for img in images]

    
    if output_format == 'gif':
        output_file = os.path.join(output_dir, f'{file_type}_{estfile}_animation.gif')
        imageio.mimsave(output_file, image_list, fps=fps)

    elif output_format == 'mp4':
        output_file = os.path.join(output_dir, f'{file_type}_{estfile}_animation.mp4')
        imageio.mimsave(output_file, image_list, fps=fps, format='mp4')



def pb_animation_wrapper(output_dir, estfile, curve_df, df_price_yield, plot_points_yr, table_breaks_yr,
                         crvtypes=['pwtf'], rate_type='parbond', opt_type='LnY', yield_to_worst=True,
                         pbtwostep=False, parmflag=True, padj=False, sqrtscale=False,
                         durscale=False, fortran=True, start_date=None, end_date=None, fps=1):
    """
    ...
    for crvtype in crvtypes:
        ...
        # Generate the PNGs for each date
        for date in plot_dates:
            ...
            yvols = None
            if 'yvols' in curve_df.columns:
                yvols = curve_df.loc[(crvtype, int(date)), 'yvols']

            # Call the plot function (which also saves the plot as a PNG)
        ...
        images = []
        for date in plot_dates:
            png_filename = os.path.join(full_path, f'{int(date)}_{rate_type}_w_act.png')
            images.append(imageio.imread(png_filename))
        
        gif_filename = os.path.join(output_dir, f'{rate_type}{estfile}_animation.gif')
...
def plot_act_single_curve_wrapper(output_dir, estfile, curve_df, df_price_yield, plot_points_yr, selected_month=12, crvtypes=['pwtf'], 
                                  rate_type='parbond', opt_type='LnY', pbtwostep=False, parmflag=True, padj=False, sqrtscale=True):
    """
...
def plot_act_pred_single_curve_wrapper(output_dir, estfile, curve_df, df_price_yield, plot_points_yr, selected_month=12,
                                       crvtypes=['pwtf'], rate_type='parbond', opt_type='LnY', yield_to_worst=True, yvols=0,
                                       pbtwostep=False, parmflag=True, padj=False, sqrtscale=True, durscale=False):
    """
    ...
    for crvtype in crvtypes:
        ...
        for date in plot_dates:
            ...
            plot_singlecurve_act_pred(output_dir=output_dir, estfile=estfile, curve=xplot_curve, plot_points_yr=plot_points_yr,
                                 rate_type=rate_type, opt_type='LnY',  yield_to_worst=yield_to_worst, yvols=yvols, df_price_yield=xdf_price_yield,
                                 pbtwostep=pbtwostep, parmflag=parmflag, padj=padj, sqrtscale=sqrtscale,durscale=durscale)
...
def plot_fwdrate_wrapper(output_dir, estfile, curve_df, plot_points_yr, taxflag=False, sqrtscale=False, yield_to_worst=True, 
        ...
        path = f"{output_dir}/{estfile}/fwd_rates"
        try:
        ...
        # sqrt root ticks and labeling
            ...
            plt.legend()
            plt.grid(True)
            full_path = f'{output_dir}/{estfile}/fwd_rates'
            os.makedirs(full_path, exist_ok=True)
            plt.savefig(f'{full_path}/{date}_fwd_rate.png')
            plt.show()
            plt.close()
