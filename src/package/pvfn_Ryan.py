...
filepath = 'D:/好好学习/UST_Project/UST-yieldcurves_2024/curve_utils/data/USTMonthly.csv'  # '../../data/1916to2024_YYYYMMDD.csv'
...
def pvCallable(tcurve, vols, opt_type, sparsenc=0, sparseyc=0., settledate=0,
...
#        callflag = sparseyc[13]
        ...
        icall = np.array(list(range(bondcount)))[callflag]     # NB - callflag must be np.array. Including the column names of all the callable bonds

        if opt_type == "LN_Yield" or opt_type == "Norm_Yield" or opt_type == "Sqrt_Yield":
            ...
            if opt_type == "LN_Yield": 
                d1=(np.log(fwdyld/strike) + 0.5*(vol**2)*expiry)/(vol*np.sqrt(expiry))
                d2=d1-vol*np.sqrt(expiry)
                call = dfcall * pvfwdann * (ss.norm.cdf(-d2)*strike-ss.norm.cdf(-d1)*fwdyld) * 100.  # Put on yield
            if opt_type == "Norm_Yield": 
                sigma = vol*np.sqrt(calldate)  # vol column here is a different one
                d = (fwdyld-strike)/sigma
                call = dfcall * pvfwdann * sigma*(-d*ss.norm.cdf(-d)+ss.norm.pdf(d)) * 100.  # Put on yield
            if opt_type == "Sqrt_Yield": 
                d1=
                d2=d1-vol*np.sqrt(expiry)
                call = dfcall * pvfwdann * (ss.norm.cdf(-d2)*strike-ss.norm.cdf(-d1)*fwdyld) * 100.
                pass
            
        if opt_type == "LN_Price": 
            ...
            d2=d1-vol*np.sqrt(expiry)
            call = dfcall * (ss.norm.cdf(d1)*bondpvfwd-ss.norm.cdf(d2)*strike)  # Call on bond
        else: 
            raise ValueError("The value of opt_type must be LN_Yield, Norm_Yield, Sqrt_Yield or LN_Price")
